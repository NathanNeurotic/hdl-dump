    ...
    if (result == OSAL_OK) {
        ...
        if (SetFilePointerEx(handle, offs, NULL, FILE_BEGIN)) /* seek to "end" */
        {
            ...
            if (buffer != NULL) {
                BOOL success;
                DWORD len;
                do { /* count the number of bytes readen after the end */
        ...
        *p = '\0';
        if (GetDiskFreeSpace(volume, &sectors_per_clust, &bytes_per_sect,
            ...
            *size_in_bytes = bytes_per_sect;
            ...
            return (OSAL_ERR);
    } else { /* if called with no volume name ask for full path */
...
int osal_query_hard_drives(osal_dlist_t **hard_drives)
{
    ...
    *hard_drives = NULL;
    ...
    for (i = 0; result == RET_OK && i < 25; ++i) {
        ...
        if (osal_open(device_name, &device, TRUE) == OSAL_OK) { /* device exists */
            u_int64_t size_in_bytes;
    ...
    *optical_drives = NULL;
    ...
    if (result == RET_OK) {
        ...
        if (req_len > 0) {
            ...
            if (buf != NULL) { /* NOTE: ignore case if device is plugged in before two calls */
                DWORD tot_len = GetLogicalDriveStrings(req_len, buf);
                if (tot_len > 0) {
                    char *p = buf;
                    do {
                        size_t len = strlen(p);
                        if (GetDriveType(p) == DRIVE_CDROM) { /* CD-/DVD-ROM drive found */
...
{
    osal_dev_t *dev;
    if (dlist->allocated == dlist->used) { /* allocate memory if necessary */
